...
public final class Form22t extends InsnFormat {
    ...
    public String insnArgString(DalvInsn dalvInsn) {
        C7160r registers = dalvInsn.getRegisters();
        return registers.m2347A(0).m2369N() + ", " + registers.m2347A(1).m2369N() + ", " + InsnFormat.branchString(dalvInsn);
    }

    @Override // com.android.p030dx.dex.code.InsnFormat
    public boolean isCompatible(DalvInsn dalvInsn) {
        C7160r registers = dalvInsn.getRegisters();
        if (!(dalvInsn instanceof TargetInsn) || registers.size() != 2 || !InsnFormat.unsignedFitsInNibble(registers.m2347A(0).m2380B()) || !InsnFormat.unsignedFitsInNibble(registers.m2347A(1).m2380B())) {
            return false;
        }
    ...
    public void writeTo(InterfaceC0428a interfaceC0428a, DalvInsn dalvInsn) {
        C7160r registers = dalvInsn.getRegisters();
        InsnFormat.write(interfaceC0428a, InsnFormat.opcodeUnit(dalvInsn, InsnFormat.makeByte(registers.m2347A(0).m2380B(), registers.m2347A(1).m2380B())), (short) ((TargetInsn) dalvInsn).getTargetOffset());
    }
}
